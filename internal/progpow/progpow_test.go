	...
	for i, tt := range tests {
		...
		for _, expectedLane := range tt.lanes {
			if len(mix)-1 < expectedLane.index {
				...
				if len(actualLane) != len(expectedLane.value) {
					t.Errorf("failed on %d: lane %d: length mismatch: have %d, want %d", i,
						expectedLane.index, len(actualLane), len(expectedLane.value))
				} else {
					for index := range expectedLane.value {
						if actualLane[index] != expectedLane.value[index] {
							t.Errorf("failed on %d: lane %d, register %d: have %d, want %d",
								i, expectedLane.index, index, actualLane[index], expectedLane.value[index])
						}
					}
				}
			}
